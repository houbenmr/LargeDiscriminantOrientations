...
class KLPT_Context:
    ...
    def KLPT(self, I, banned_integer, T=None, returnElem=False, variant=None):
        r"""
        ...
        while not nu:
            ...
            if not T:
                ...
                if variant == 'simple':
                    T, facToExt = choose_torsion(p, self.q, N, max(p*N**4, p**3), banned_integer)
                    fac = list(factor(T))
                    S = None
                    f = None
                elif variant == 'slide':
                    T, facToExt = choose_torsion(p, self.q, N, p**(4.2/2), banned_integer, newconst=(0.31, 15*1.17, 15*0.46, 15*0.01))
