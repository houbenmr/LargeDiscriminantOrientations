...
def chain_iso(kernelPointsIn, E, pushPoints = []):
    r"""
    ...
    while kernelPoints:
        ...
        if len(kernelPoints) > 1:
            kernelPoints = [tup for _,tups in parallel(cpu_count() - 2)(push)(chunks) for tup in tups] # - 2 to not use all cores
        else:
            kernelPoints = push(kernelPoints)

        pushPoints = [P.push(phi) for P in pushPoints]

        philist.append(phi)

    if pushPoints:
        return EllipticCurveHom_composite.from_factors(philist), pushPoints
    else:
...
class Deuring_Context:
    ...
    def _precompute_torsion_bases(self, Eles):
        from os import cpu_count
        for (Ele,_), (EE,emb, P,Q) in parallel(cpu_count() - 2)(self.genTorsionBasis)(Eles): # - 2 to not use all cores
    ...
    def IdealToIsogeny(self, I, specificTorsion=0, pushPoints = []):
        r"""
        ...
        return chain_iso(kerGens, self.E0, pushPoints = pushPoints)
...
def constructive_deuring(I, E0, iota, banned_integer, variant=None, pushPoints = []):
    r"""
    ...
    alpha, J, facToExt, T, S, f = KLPT_ctx.KLPT(I, banned_integer, returnElem=True, variant=variant)
...
#        print()
    ...
    if S:
        ...
        verbose("Using one-shot ideal-isogeny translation")
        if pushPoints:
            phi_I, pushPoints = Deuring_ctx.IdealToIsogeny(J, pushPoints = pushPoints)
        else:
            phi_I = Deuring_ctx.IdealToIsogeny(J, pushPoints = pushPoints)

    if pushPoints:
        return phi_I.codomain(), phi_I, alpha, J, Deuring_ctx, pushPoints
    else:
        return phi_I.codomain(), phi_I, alpha, J, Deuring_ctx
